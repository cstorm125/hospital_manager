...
plot_prophet = function(result,w=690,h=290,scaler=1/1e6){
...
renderPlotly({
  ...
  g = ggplot(results, aes(x=ds,y=yhat_per,fill=variable,group=variable)) + 
    ...
    scale_x_discrete(breaks=results$ds[seq(1,length(results$ds),4)],labels=function(x) substr(x,1,4)) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1),
          legend.title=element_blank())
  ggplotly(g,width=690,height=290)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'total_payroll_expenses')[[1]],w=370,h=175)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'employee_benefit_expenses')[[1]],w=370,h=175)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'bad_debts_expenses')[[1]],w=370,h=175)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'supply_expenses')[[1]],w=370,h=175)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'total_contractual_total')[[1]],w=370,h=175)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'depre_expenses')[[1]],w=370,h=175)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'interest_expenses')[[1]],w=370,h=175)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'total_charity_care')[[1]],w=370,h=175)
})
```
...
```{r}
renderPlotly({
  plot_prophet(gen_forecast(df(),'other_operating_expenses')[[1]],w=370,h=175)
})
```
...
fluidRow(
      ...
      column(6,
        numericInput('add_ppe','Add PPE (M)',value = 0,width=175),
        numericInput('add_interest','Interest Rate',value = 0,width=175),
        numericInput('add_other_operating','Other Operating Expenses (M)',value = 0,width=175)
      )
)
...
margin_call = reactive({
  control_df = df()[,c('ds','patient_margin','operating_margin','net_margin','total_assets')] %>%
  ...
  #total revenue
  alt_df$total_patient_revenue = gen_forecast(df(),'total_patient_revenue')[[1]][,input$add_revenue]
  ...
  #calculate metrics
  alt_df = alt_df %>%
    mutate(patient_margin = total_patient_revenue - (total_contractual_total + 
                                               total_payroll_expenses+employee_benefit_expenses+
                                               supply_expenses+bad_debts_expenses),
           operating_margin = total_revenue - (total_operating_expenses + total_contractual_total +
                                               total_charity_care),
           net_margin = operating_margin + net_non_operating + tax_revenue,
           roa = net_margin / total_assets)
  alt_df = alt_df[,c('ds','test_flag','patient_margin','operating_margin','net_margin','total_assets','roa')]
